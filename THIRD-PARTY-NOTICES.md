...
- `Portions adapted from` **零命中**——按本项目自己的约定（见清单文档「使用规范」表），
  这是「未发生标记为复制的代码块」的直接证据。
...
- 结论：**当前无需登记任何「代码衍生」条目**。上表「实际关系」一栏如实标注为
  「参考思路」或「参考思路 + 结构对齐」，没有为凑齐模板而虚构复制记录。
...
**已知缺口（如实记录，不掩饰）**：前端侧目前**没有**等价于 `cargo deny` 的
机械化许可扫描工具接入（例如 `license-checker` 之类），上述许可证信息来自
各包的公开发行信息，不是本次核查用工具逐包验证的结果。这与 Rust 侧「有
`deny.toml` 作机械保证」不是同一可信度级别，留作后续改进项。
...
- **角色 / 物品图标**：风险高，**绝不打包进仓库**。运行时按 URL 拼接从游戏厂商
  自己的静态资源域名加载（例如异环走 `webstatic.tajiduo.com`），仓库里只存
  拼接规则，不存图片本身。

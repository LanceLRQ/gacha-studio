...
//   已证伪 · HC-3 红时拒绝豁免（2026-08-14 补证）。构造方式：M3 两个提交落地后
//            工作区干净、head == HEAD，此时手改 plugins.bundle.js 并提交，使
//            "已提交内容 ≠ codegen 产物"，随后 `git reset --hard` 回退。实跑输出
//            确认拒绝豁免，且理由文案与前两道守卫可区分（"HC-3（codegen 一致性）
//            未通过"）。注意守卫顺序在这里是关键：dirty 检查发生在跑 codegen 之前，
//            所以这次触发的确实是第三道守卫，而不是被 codegen 重跑弄脏工作区后
//            由第二道守卫顺手报的红。
//

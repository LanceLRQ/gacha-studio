...
/**
 * 日志解混淆参数——{@link CredentialSource} 的 `kind: "logFile"` 分支用，
 * 声明「怎么把混淆过的日志字节还原成明文」这组**参数**，不声明算法本身。
 * 目前唯一的真实样本是鸣潮 `Client.log`：跳过前 3 字节，此后逐字节按
 * **该字节自身的值**是奇数还是偶数，异或不同的掩码。
 *
 * ## 为什么参数在 TS、算法在 Rust
 *
 * 判断依据是本项目的能力边界测试（`docs/_internal/milestones/00-实施总览.md`
 * §11.5）：**「这段逻辑换一个游戏还成立吗？」**——异或解混淆的**算法框架**
 * （逐字节读取、位运算、文件 IO）换一个游戏依然成立，归 Rust；但「跳过前
 * 3 字节、按字节值奇偶分别异或 `0xA5`/`0xEF`」这组**具体参数**只对鸣潮成立，
 * 换一个游戏就是另一组数字（甚至可能不是异或而是别的编码），归 TS 声明。
 *
 * ## 两个反向错误
 *
 * 完整裁定见 `docs/_internal/milestones/03-M2-鸣潮插件与抽象证伪.md` §4.7。
 *
 * 1. **把整个解混淆逻辑写进 TS** —— 直接破 HC-2。TS 侧运行环境**物理上不
 *    提供** `fs`，插件代码读不了文件；硬做的唯一方式是先在 Rust 侧把原始
 *    字节数组传给 TS 让插件自己解析，这等价于把二进制跨 IPC 交给插件，违反
 *    「二进制不上浮」——TS 侧只应见到解混淆后的结构化数据（URL 字符串），
 *    见不到中间字节。
 * 2. **把解混淆参数硬编码进 Rust** —— 破坏「新游戏只改插件」的目标。
 *    `0xA5`/`0xEF`/跳 3 字节是鸣潮专属的编码参数，若写死在 Rust 里，下一个
 *    用同类混淆手法但参数不同的游戏就无法通过改插件接入，只能改 Rust 主程序。
 *
 * ## `maskWhenOdd`/`maskWhenEven` 判据：字节值本身，与下标无关
 *
 * ⚠️ **奇偶判断的对象是该字节自身的值，不是它在文件中的下标**——这两种写法
 * 都能跑、都不报错，但产出完全不同，且错的那种在小样本测试里可能碰巧通过。
 * 源码依据 `WWGachaExport/ViewModels/Dialogs/UpdateGachaDataDialogViewModel.cs`
 * 第 118~123 行：`byte b = encrypted[i]; if ((b & 1) == 1) b ^= 0xA5; else
 * b ^= 0xEF;`——分支判的是 `b`（字节值），循环变量 `i` 只用于取值和写回。
 *
 * 之所以要把这条钉死：鸣潮这部分的二手转述已经被证伪过一轮。`research/01`
 * 原记载的凭据路径与格式（`debug.log`、明文、PC 需抓包）经源码校准后确认
 * 三处全错（实际是 `Client.log` + 异或混淆 + PC 不需抓包），完整校准记录见
 * `00-实施总览.md` §7.8。异或判据本身不在那三处之列——原记载认为日志是明文，
 * 压根没描述过解码——但同一份材料已经错过三处，这条同样只认源码，不认转述。
 *
 * ## 为什么是单变体判别联合，不是扁平结构体
 *
 * 解混淆是一个方案家族（下一个游戏可能是 RC4、滚动异或、base64 等），`kind`
 * 标签在 manifest 里本身就是文档，将来加第二种方案是非破坏性变更。
 *
 * ⚠️ **现在只保留一个变体是刻意的，不要提前加第二个**——本项目有明确教训：
 * 占位判别联合分支比占位 crate 更阴险，它能过类型检查、过所有门禁、甚至被
 * 文档引用为设计依据，却永远跑不起来（`CollectConfig` 曾经的 `logScan`
 * 分支与本类型是两个从未互相引用、也从未被任何代码路径跑过的重复扩展点，
 * M2-S2 已删除，见 {@link CollectConfig} 类型定义上的说明）。样本数为 1
 * 时按三次法则不预先设计第二种方案，等第二个需要解混淆的游戏出现再加。
 */
export type LogDecodeSpec = {
  kind: "xorByLowBit";
  /** 跳过文件开头的字节数。 */
  skipBytes: number;
...
export type CredentialSource =
  ...
  | {
      ...
      urlPattern: RegExp;
      /**
       * 解混淆参数。省略表示日志是明文，不需要解码。
       */
      decode?: LogDecodeSpec;
    }
  | { kind: "manual" };

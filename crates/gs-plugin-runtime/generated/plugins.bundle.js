	...
	*/
	...
	const manifest = {
		...
		pityGroups: WUWA_POOL_TYPES.flatMap((pool) => {
			const fiveStarGuarantee = pool.name.includes("武器") ? { kind: "alwaysRateUp" } : pool.name.includes("角色") ? { kind: "fiftyFifty" } : { kind: "none" };
			return [{
				key: `${pool.id}-5star`,
				members: [pool.id],
		...
		time: { rawTimeConvention: "serverLocal" },
		preconditions: [{
